...
public class SRLA {
	...
	int num_of_evaluations,total_num_of_evaluations,num_iterations,v;
	int length_coe_list;
	
    public SRLA(WindFarmLayoutEvaluator evaluator) {
		length_coe_list = 10;
		number_of_LLHs = 7;
		wfle = evaluator;
		rand = new Random();
	...
	public void run() {
		...
		best_obj = obj;
		list_of_costs = new double[length_coe_list];
		for (int i = 0; i<length_coe_list; ++i) {
			list_of_costs[i] = obj;
		}
		num_iterations = 0;
		// Hyper-heuristic 
		while(num_of_evaluations < total_num_of_evaluations) {
			...
			v = num_iterations % length_coe_list;
			
			if ( obj <= list_of_costs[v] ){
				if ( obj < best_obj ){
					util_LLH[h]++;
					for (int i=0; i<grid.size(); i++) {
						best_solution[i] = solution[i];
			...
			list_of_costs[v] = obj;
			num_iterations++;
			System.out.println(obj + "\t" + best_obj);
		}
